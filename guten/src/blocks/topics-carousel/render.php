<?php
/**
 * Template for rendering the Topics Carousel block.
 *
 * @package domca
 */
...
$domca_topics_carousel_item_bg_svg = '<svg
		preserveAspectRatio="none"
		aria-hidden="true"
		viewBox="0 0 466 166"
		fill="none"
		class="' . esc_attr( $domca_topics_carousel_base_class . '__slide-decor' ) . '"
	>
		<path
...
<section
		...
		echo $domca_topics_carousel_wrapper_attributes;
		?>
>
	<div class="<?php echo esc_attr( $domca_topics_carousel_base_class . '__wrap dm-container' ); ?>">
		<?php if ( ! empty( $domca_topics_carousel_title ) ) : ?>
			<h2 class="<?php echo esc_attr( $domca_topics_carousel_base_class . '__title dm-heading dm-heading-h2' ); ?>">
				<?php echo wp_kses_post( $domca_topics_carousel_title ); ?>
			</h2>
		<?php endif; ?>

		<?php if ( ! empty( $domca_topics_carousel_slides ) && is_array( $domca_topics_carousel_slides ) ) : ?>
			<div class="<?php echo esc_attr( $domca_topics_carousel_base_class . '__slides' ); ?>">

				<?php
				if ( count( $domca_topics_carousel_slides ) > 3 ) {
					echo '<div class="swiper-wrapper" role="list">';
				}
				?>

				<?php foreach ( $domca_topics_carousel_slides as $domca_slide ) : ?>
					<?php
					$domca_slide_title = is_array( $domca_slide ) ? ( $domca_slide['title'] ?? '' ) : '';
					$domca_slide_text  = is_array( $domca_slide ) ? ( $domca_slide['text'] ?? '' ) : ( is_string( $domca_slide ) ? $domca_slide : '' );
					?>
					<div class="<?php echo esc_attr( $domca_topics_carousel_base_class . '__slide' . ( count( $domca_topics_carousel_slides ) > 3 ? ' swiper-slide' : '' ) ); ?>">
						<?php echo wp_kses( $domca_topics_carousel_item_bg_svg, domca_get_svg_allowed_html() ); ?>

						<?php if ( ! empty( $domca_slide_title ) ) : ?>
							<p class="<?php echo esc_attr( $domca_topics_carousel_base_class . '__slide-title' ); ?>">
								<?php echo wp_kses_post( $domca_slide_title ); ?>
							</p>
						<?php endif; ?>

						<?php if ( ! empty( $domca_slide_text ) ) : ?>
							<p class="<?php echo esc_attr( $domca_topics_carousel_base_class . '__slide-text' ); ?>">
								<?php echo wp_kses_post( $domca_slide_text ); ?>
							</p>
						<?php endif; ?>
					</div>
				<?php endforeach; ?>
				...
				if ( count( $domca_topics_carousel_slides ) > 3 ) {
					echo '</div>';
					echo '<div class="' . esc_attr( $domca_topics_carousel_base_class ) . '__pagination swiper-pagination"></div>';
				}
				?>
			</div>
		<?php endif; ?>

		<?php if ( ! empty( $domca_topics_carousel_button ) ) : ?>
			<p class="<?php echo esc_attr( $domca_topics_carousel_base_class . '__button dm-button dm-button-primary' ); ?>">
				<?php echo wp_kses_post( $domca_topics_carousel_button ); ?>
			</p>
		<?php endif; ?>
	</div>
</section>
