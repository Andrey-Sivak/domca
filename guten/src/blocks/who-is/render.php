<?php
/**
 * Template for rendering the Who Is block.
 *
 * @package domca
 */
...
$domca_whois_title  = $attributes['title'] ?? '';
$domca_whois_items  = ( isset( $attributes['items'] ) && is_array( $attributes['items'] ) ) ? $attributes['items'] : array();
$domca_whois_button = $attributes['button'] ?? '';

$domca_whois_base_class = 'wp-block-domca-who-is';

$domca_whois_wrapper_attributes = get_block_wrapper_attributes(
	array(
		'class' => 'dm-wrap',
	)
);
...
$domca_whois_item_bg_svg = '<svg
		preserveAspectRatio="none"
		aria-hidden="true"
		viewBox="0 0 466 166"
		fill="none"
		class="' . esc_attr( $domca_whois_base_class . '__slide-decor' ) . '"
	>
		<path
...
<section
		...
		echo $domca_whois_wrapper_attributes;
		?>
>
	<div class="<?php echo esc_attr( $domca_whois_base_class . '__wrap dm-container' ); ?>">
		<?php if ( ! empty( $domca_whois_title ) ) : ?>
			<p class="<?php echo esc_attr( $domca_whois_base_class . '__title dm-heading dm-heading-h2' ); ?>">
				<?php echo wp_kses_post( $domca_whois_title ); ?>
			</p>
		<?php endif; ?>

		<?php if ( ! empty( $domca_whois_items ) && is_array( $domca_whois_items ) ) : ?>
			<div class="<?php echo esc_attr( $domca_whois_base_class . '__slides' ); ?>">

				<?php
				if ( count( $domca_whois_items ) > 3 ) {
					echo '<div class="swiper-wrapper" role="list">';
				}
				?>

				<?php foreach ( $domca_whois_items as $domca_item ) : ?>
					<?php
					$domca_item_text = '';
					$domca_img_id    = 0;
					$domca_img_url   = '';

					if ( is_array( $domca_item ) ) {
						$domca_item_text = $domca_item['text'] ?? '';

						// Support both "icon" and "image" keys.
						if ( ! empty( $domca_item['icon'] ) && is_array( $domca_item['icon'] ) ) {
							$domca_img_id  = isset( $domca_item['icon']['id'] ) ? (int) $domca_item['icon']['id'] : 0;
							$domca_img_url = isset( $domca_item['icon']['url'] ) ? (string) $domca_item['icon']['url'] : '';
						} elseif ( ! empty( $domca_item['image'] ) && is_array( $domca_item['image'] ) ) {
							$domca_img_id  = isset( $domca_item['image']['id'] ) ? (int) $domca_item['image']['id'] : 0;
							$domca_img_url = isset( $domca_item['image']['url'] ) ? (string) $domca_item['image']['url'] : '';
						} elseif ( isset( $domca_item['image'] ) && is_string( $domca_item['image'] ) ) {
							$domca_img_url = $domca_item['image'];
						}
					}

					// Skip completely empty slide.
					if ( '' === trim( (string) $domca_item_text ) && ! $domca_img_id && '' === $domca_img_url ) {
						continue;
					}
					?>
					<div class="<?php echo esc_attr( $domca_whois_base_class . '__slide ' . ( count( $domca_whois_items ) > 3 ? ' swiper-slide' : '' ) ); ?>">
						<?php echo wp_kses( $domca_whois_item_bg_svg, domca_get_svg_allowed_html() ); ?>

						<?php if ( $domca_img_id ) : ?>
						<figure class="<?php echo esc_attr( $domca_whois_base_class ); ?>__slide-image">
							<?php echo wp_get_attachment_image( $domca_img_id, 'full', false, array() ); ?>
						</figure>
						<?php endif; ?>

						<?php if ( ! empty( $domca_item_text ) ) : ?>
							<p class="<?php echo esc_attr( $domca_whois_base_class ); ?>__slide-text">
				...
				if ( count( $domca_whois_items ) > 3 ) {
					echo '</div>';
					echo '<div class="' . esc_attr( $domca_whois_base_class ) . '__pagination swiper-pagination"></div>';
				}
				?>
		...
		<?php if ( ! empty( $domca_whois_button ) ) : ?>
			<p class="<?php echo esc_attr( $domca_whois_base_class . '__button dm-button dm-button-primary' ); ?>">
				<?php echo wp_kses_post( $domca_whois_button ); ?>
			</p>
		<?php endif; ?>
	</div>
</section>
